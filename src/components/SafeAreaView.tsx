...
export default function SafeAreaView(
  { className, children, ...props }: SafeAreaViewProps,
) {
  const defaultSafeArea =
    "h-full pt-[var(--ion-safe-area-top)] pr-[var(--ion-safe-area-right)] pb-[var(--ion-safe-area-bottom)] pl-[var(--ion-safe-area-left)]";

  return (
    <div className={`${defaultSafeArea} ${className}`} {...props}>
      {children}
    </div>
  );
}
